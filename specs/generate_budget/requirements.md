...
GIVEN Mateo is authenticated on the dashboard
...
GIVEN Mateo is on the budget parameters settings
...
WHEN Mateo enters a manual final price
...
WHEN Mateo marks it as approved
THEN the status changes to `approved`
AND the order status changes to `approved`

GIVEN the budget is in `sent` status
WHEN Mateo marks it as rejected
THEN the status changes to `rejected`
AND Mateo can create a new budget version for the same order

## R10. Budget parameters — Invalid values

GIVEN Mateo enters a negative value or zero for any numeric parameter
...
GIVEN Mateo enters grams ≤ 0 or hours < 0
...
WHEN Mateo views the order detail
